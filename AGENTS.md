...
1. Read the JSON trigger payload. It must contain an Instagram URL in `url`.
   It may also contain a `requestId` UUID generated by the iPhone Shortcut.
...
7. POST this JSON shape to the URL in `SHEETS_WEBHOOK_URL`:
   ...
   ```json
   {
     ...
     "requestId": "<trigger requestId, or omit when absent>",
     "inboxRow": [],
     "category": "Places",
     "categoryHeaders": [],
     "categoryRow": []
   ...
   When the trigger includes `requestId`, copy it unchanged into the Sheet
   webhook payload. Never invent or modify a request ID.
